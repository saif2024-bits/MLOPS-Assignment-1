...
class TestHeartDiseasePredictor:
    ...
    def setup_class(cls):
        ...
        results = trainer.train_model(model, X_train, y_train, X_test, y_test, model_name='XGBoost')
    ...
    def test_load_models(self):
        ...
        assert self.predictor.preprocessing_pipeline is not None, "Pipeline should be loaded"
    ...
    def test_predict_without_loading_model(self):
        ...
        patient_data = {'age': 63, 'sex': 1, 'cp': 1, 'trestbps': 145,
                       'chol': 233, 'fbs': 1, 'restecg': 2, 'thalach': 150,
                       'exang': 0, 'oldpeak': 2.3, 'slope': 3, 'ca': 0, 'thal': 6}
    ...
    def test_predict_with_missing_features(self):
        ...
        patient_data = {
            'age': 63, 'sex': 1, 'cp': 1, 'trestbps': 145,
            'chol': 233, 'fbs': 1, 'restecg': 2, 'thalach': 150,
            'exang': 0, 'oldpeak': 2.3, 'slope': 3, 'ca': 0
        }
...
class TestModelPackageSaving:
    ...
    def test_save_model_package(self):
        ...
        results = trainer.train_model(model, X_train, y_train, X_test, y_test, model_name='Logistic Regression')
...
class TestPredictionValidation:
    ...
    def test_extreme_age_values(self):
        ...
        young_patient = {
            'age': 20, 'sex': 1, 'cp': 1, 'trestbps': 120,
            'chol': 200, 'fbs': 0, 'restecg': 0, 'thalach': 180,
            'exang': 0, 'oldpeak': 0, 'slope': 1, 'ca': 0, 'thal': 3
        }
        result = self.predictor.predict(young_patient)
        assert result['prediction'] in [0, 1]

        old_patient = {
            'age': 80, 'sex': 1, 'cp': 1, 'trestbps': 150,
            'chol': 250, 'fbs': 1, 'restecg': 2, 'thalach': 120,
            'exang': 1, 'oldpeak': 3, 'slope': 3, 'ca': 2, 'thal': 6
        }
