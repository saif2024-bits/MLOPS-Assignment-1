...
class TestModelTrainer:
    ...
    def test_cross_validation(self):
        ...
        for model_name in ["Logistic Regression", "Random Forest", "XGBoost"]:
            ...
            assert (
                "accuracy_mean" in cv_results[model_name]
            ), "Should have accuracy mean"
            ...
            assert (
                cv_results[model_name]["accuracy_mean"] > 0.5
            ), "CV accuracy should be reasonable"
    ...
    def test_train_all_models(self):
        ...
        for model_name in expected_models:
            assert (
                model_name in self.trainer.results
            ), f"Should have results for {model_name}"
    ...
    def test_best_model_selection(self):
        ...
        best_name = max(
            self.trainer.results.items(), key=lambda x: x[1]["test_roc_auc"]
        )[0]
    ...
    def test_model_predictions_shape(self):
        ...
        assert (
            predictions.shape[0] == self.X_test.shape[0]
        ), "Predictions shape should match test size"
        assert probabilities.shape == (
            self.X_test.shape[0],
            2,
        ), "Probabilities shape should be (n_samples, 2)"
