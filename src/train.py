...
class ModelTrainer:
    ...
    def plot_model_comparison(self, save_path=None):
        ...
        for idx, metric in enumerate(metrics):
            ...
            axes[idx].bar(range(len(model_names)), scores, color=['steelblue', 'coral', 'lightgreen'])
        ...
        for idx, model_name in enumerate(model_names[:3]):
            if idx + 5 < len(axes):
                ...
                sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', ax=axes[idx + 5],
                           cbar=False, square=True)
                axes[idx + 5].set_title(f'{model_name} - Confusion Matrix', fontweight='bold')
